...
## Codex MiniMax Hybrid Workflow

Use the MiniMax + Codex/GPT workflow to reduce cost with minimal user friction:

1. For discussion, reasoning, planning, design, architecture options, tradeoff analysis, implementation strategy, and review prompts, use MiniMax first by default.
2. For long discussion or analysis, run `codex-minimax ask --output minimax-answer.md "QUESTION"` and read the saved file instead of printing the full answer into chat.
3. Before implementation tasks, run `codex-minimax plan "TASK"` and read `plans/latest-minimax-plan.md`.
4. Codex/GPT performs code reading, edits, testing, debugging, and final verification.
5. For meaningful diffs, optionally run `codex-minimax review` and read `reviews/latest-minimax-review.md`.
6. Treat MiniMax output as advisory input; Codex/GPT makes final implementation decisions.
...
Do not print secrets from `~/.config/codex-minimax/env`.
